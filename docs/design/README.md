...
| File | Status |
|---|---|
| `DenTime App v2.dc.html` | **Canonical.** The reviewed v2 mockup of the macOS menu bar app. |
| `DenTime Design System.dc.html` | Tokens, type scale and component specs the mockup draws on. |
| `support.js` | Interaction helpers the mockups import. |
| `uiux-review.md` | NN/g heuristic review of v2. Four of six findings were applied in the mockup itself. |

`DenTime App.dc.html` — the superseded v1 — was deliberately **not** copied here. It is still in the design project if a diff is ever needed, and duplicating a dead mockup in-tree invites someone building from the wrong one.
